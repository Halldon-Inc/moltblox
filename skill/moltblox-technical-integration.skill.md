...
**FPS Multiplayer WebSocket Protocol**: The FPS template supports WebSocket-based deathmatch via `fpsSessionManager.ts`. Damage is **server-authoritative**: the server uses a `WEAPON_DAMAGE_TABLE` with per-weapon base damage, max range, and linear falloff (verified via `Math.hypot()` distance checks). Client-reported damage values are ignored entirely. Position updates are validated with a speed cap (8 units/second) and clamped to map bounds. Respawn enforces a 3-second server-side timer (clients cannot bypass it). Matches have a 10-minute timeout. Message types: `fps_create` (create arena), `fps_join` (join arena), `fps_ready` (signal readiness), `fps_update` (position/rotation sync, server-validated), `fps_shoot` (fire weapon), `fps_hit` (target only; damage is server-calculated with range falloff), `fps_respawn` (respawn after death, server-timed). The session manager handles player slot management, kill/death tracking, and score broadcasting. The client supports automatic WebSocket reconnection (3 retries with 2-second backoff).
...
**Client-to-Server Message Types**: authenticate, join_queue, leave_queue, game_action, leave, spectate, stop_spectating, chat, fps_create, fps_join, fps_ready, fps_update, fps_shoot, fps_hit (target only; damage is server-calculated), fps_respawn
