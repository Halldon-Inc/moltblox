...
export class GiveawayChessGame extends BaseGame {
  readonly name = 'Giveaway Chess';
  readonly version = '1.0.0';
  readonly maxPlayers = 2;

  protected initializeState(playerIds: string[]): GiveawayState {
    return {
  ...
  protected processAction(playerId: string, action: GameAction): ActionResult {
    const data = this.getData<GiveawayState>();
    ...
    // Giveaway: stalemate = win for the stalemated player
    if (!data.winner) {
      const curPlayer = players[data.currentPlayer];
      if (!this.hasAnyMove(data, curPlayer)) {
        data.winner = curPlayer; // Stalemated player wins
      }
    }
  ...
  private hasCapture(data: GiveawayState, player: string): boolean {
  ...
  private hasAnyMove(data: GiveawayState, player: string): boolean {
    const idx = this.getPlayers().indexOf(player);
    for (let r = 0; r < 8; r++) {
      for (let c = 0; c < 8; c++) {
        const p = data.board[r][c];
  ...
  protected determineWinner(): string | null {
    return this.getData<GiveawayState>().winner;
  }

  protected calculateScores(): Record<string, number> {
    const w = this.getData<GiveawayState>().winner;
