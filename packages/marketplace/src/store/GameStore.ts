...
import type { PublishedGame, GameItem, OwnedItem, Purchase } from '@moltblox/protocol';
...
const KEYS = {
  ...
  inventoryByGame: (playerId: string, gameId: string) => `inventory:${playerId}:game:${gameId}`,
  ...
  // Revenue
  revenueDaily: (gameId: string, date: string) => `revenue:${gameId}:${date}`,
...
export class GameStore {
  ...
  async updateGame(gameId: string, updates: Partial<StoredGame>): Promise<void> {
    ...
    await this.redis.set(this.key(KEYS.game(gameId)), JSON.stringify(updated));
  }
  ...
  async getAllGameIds(limit: number = 100, offset: number = 0): Promise<string[]> {
    return this.redis.zrevrange(this.key(KEYS.allGames), offset, offset + limit - 1);
  }
  ...
  async updateItem(itemId: string, updates: Partial<GameItem>): Promise<void> {
    ...
    await this.redis.set(this.key(KEYS.item(itemId)), JSON.stringify(updated));
  }

  async getItemsByGame(gameId: string): Promise<GameItem[]> {
    const itemIds = await this.redis.smembers(this.key(KEYS.itemsByGame(gameId)));
  ...
  async savePurchase(purchase: Purchase): Promise<void> {
    await this.redis
      .multi()
      .set(this.key(KEYS.purchase(purchase.purchaseId)), JSON.stringify(purchase))
      .lpush(this.key(KEYS.purchasesByPlayer(purchase.buyerBotId)), purchase.purchaseId)
      .lpush(this.key(KEYS.purchasesByGame(purchase.gameId)), purchase.purchaseId)
      .exec();
  }
  ...
  async addToInventory(playerId: string, ownedItem: OwnedItem): Promise<void> {
    const inventoryKey = this.key(KEYS.inventory(playerId));
    const gameInventoryKey = this.key(KEYS.inventoryByGame(playerId, ownedItem.gameId));
  ...
  async getInventoryForGame(playerId: string, gameId: string): Promise<OwnedItem[]> {
    const itemIds = await this.redis.smembers(this.key(KEYS.inventoryByGame(playerId, gameId)));
  ...
  async incrementGameStat(gameId: string, stat: string, amount: number = 1): Promise<void> {
    await this.redis.hincrby(this.key(KEYS.gameStats(gameId)), stat, amount);
  }
  ...
  async recordRevenue(gameId: string, creatorId: string, amount: string): Promise<void> {
    const date = new Date().toISOString().split('T')[0];

    await this.redis
      .multi()
      // Daily revenue for game
      .incrbyfloat(this.key(KEYS.revenueDaily(gameId, date)), parseFloat(amount))
      // Total creator revenue
      .incrbyfloat(this.key(KEYS.revenueCreator(creatorId)), parseFloat(amount))
      .exec();
  }

  async getCreatorTotalRevenue(creatorId: string): Promise<string> {
    const revenue = await this.redis.get(this.key(KEYS.revenueCreator(creatorId)));
    return revenue || '0';
  }
  ...
  async updateTrendingScore(gameId: string, score: number): Promise<void> {
